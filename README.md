...
![Version](https://img.shields.io/badge/version-v3.2.0-2d68df)
...
| 🛡️ **数据安全** | 自动保护 | 升级结构前自动备份；工具页可检查、手动备份和选择恢复 |
...
每条投递还可以保存「投递日期、网申截止、下一步时间、最后跟进、投递来源、岗位原始链接、JD 原文快照」。其中链接用于回访原页面，JD 快照直接保存在本地 SQLite 和 Excel 镜像中，因此岗位下架后仍能用于简历与面试复盘。
