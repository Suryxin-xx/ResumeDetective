...
> [!TIP]
> **v4.3.2 更新：** 首页新增重点面试提醒，面试记录支持时间、形式、会议链接和安排备忘；同时重排新建投递表单、简历汇总列表和面试复盘页面。旧数据库会以增量方式升级到 schema v9，不覆盖既有投递与简历数据。
...
| 总览与行动 | 用四个核心数字、需要关注、招聘阶段和最近变化判断推进重点；业务面试与 HR 面会集中展示时间、形式、链接和备忘 |
...
```powershell
.\scripts\atuo.bat -Version 4.3.2
```
