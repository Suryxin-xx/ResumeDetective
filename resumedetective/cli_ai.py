...
def _create_runtime_home(api_key: str | None = None) -> Path:
    ...
    env_src = _get_local_env_path()
    example_src = REASONIX_DATA_DIR / ".env.example"

    if cfg_src.exists():
        shutil.copy2(cfg_src, runtime_home / "config.toml")
    if env_src.exists():
        shutil.copy2(env_src, runtime_home / ".env")
    if example_src.exists():
        shutil.copy2(example_src, runtime_home / ".env.example")
    return runtime_home
...
def _reasonix_subprocess_env(api_key: str | None = None, runtime_home: Path | None = None) -> dict:
    ...
    """
    home = runtime_home or REASONIX_DATA_DIR
    if runtime_home is None:
        sync_local_reasonix_env(api_key)
    env = os.environ.copy()
    env["REASONIX_HOME"] = str(home)
    return env
...
def call_reasonix_blocking(prompt: str, model: str = None, cli_path: str = None) -> tuple:
    ...
    try:
        result = subprocess.run(
            ...
            env=_reasonix_subprocess_env(runtime_home=runtime_home),
            creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, "CREATE_NO_WINDOW") else 0,
        )
        if result.returncode == 0:
            output = result.stdout.strip()
            return True, output
        err_text = result.stderr.strip() or f"退出码 {result.returncode}"
...
def upgrade_reasonix() -> tuple:
    ...
    try:
        ...
        if result.returncode == 0:
            output = result.stdout.strip() or "升级成功"
            return True, output
        if new_exe.exists():
