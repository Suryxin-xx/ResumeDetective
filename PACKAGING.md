...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build_windows.ps1 -Version 4.7.0
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build_windows.ps1 -Version 4.7.0 -ArchiveExisting
```

## 输出结构

```text
ResumeDetective\releases\v4.7.0\
├── ResumeDetective\
│   ├── ResumeDetective.exe
...
不要上传整个 `releases\v4.7.0\ResumeDetective` 文件夹、真实 `data`、`backups` 或 `.env`。GitHub 会自动提供 Source code ZIP/TAR，不需要再手工打源码包。
