...
function hashSelection() { const params = new URLSearchParams(window.location.hash.split("?")[1] || ""); return { status: params.get("status") || "", stage: params.get("stage") || "", application: Number(params.get("application") || 0) }; }
...
export default function ApplicationsPage({ data, refresh, go, newSignal, consumeNewSignal }: PageProps & { newSignal: number; consumeNewSignal: () => void }) {
  ...
  useEffect(() => {
    const syncHash = () => { const selection = hashSelection(); if (selection.status) setFilter(selection.status); if (selection.stage) setStageFilter(selection.stage); if (selection.application) { setFilter("全部"); setDetailId(selection.application); requestAnimationFrame(() => document.getElementById(`application-${selection.application}`)?.scrollIntoView({ behavior: "smooth", block: "center" })); } };
  ...
  }).sort((a,b)=>{ const terminalOrder=Number(terminalStatuses.has(a.currentStatus))-Number(terminalStatuses.has(b.currentStatus)); if(terminalOrder)return terminalOrder; return sort==="company"?a.companyName.localeCompare(b.companyName,"zh-CN"):sort==="priority"?b.priority-a.priority:new Date(b.statusUpdateTime).getTime()-new Date(a.statusUpdateTime).getTime(); }), [data.applications, query, filter, categoryFilter, stageFilter, sort, hideTerminal]);
  ...
  const clearFilters=()=>{setQuery("");setFilter("全部");setCategoryFilter("全部类型");setStageFilter("全部进展");setSort("updated");setHideTerminal(false);setSelected(new Set());setDetailId(null)};
...
function QuickProgressModal({item,onClose,refresh}:{item:Application;onClose:()=>void;refresh:()=>Promise<void>}){
  ...
  async function update(event:React.FormEvent<HTMLFormElement>){
    ...
    const payload={
      currentStatus:String(values.currentStatus),stageState:String(values.stageState),nextAction:item.nextAction,
      ...
      nextActionDueAt:item.nextActionDueAt,lastFollowUpAt:item.lastFollowUpAt,
    };
  ...
  return <Modal title="快速更新进度" subtitle={`${item.companyName} · ${item.positionName}`} onClose={onClose}><form onSubmit={update} className="quick-progress-form">
    <Field label="当前环节"><select name="currentStatus" defaultValue={item.currentStatus} autoFocus>{(statuses.includes(item.currentStatus)?statuses:[item.currentStatus,...statuses]).map(value=><option key={value}>{value}</option>)}</select></Field>
    <Field label="这一步现在怎样了？" hint="只描述当前一步；进入下一环节时再修改上方选项。"><select name="stageState" defaultValue={item.stageState}>{stageStateChoices(item.stageState).map(choice=><option key={choice.value} value={choice.value}>{choice.label}</option>)}</select></Field>
