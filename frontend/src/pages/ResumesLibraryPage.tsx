...
export default function ResumesLibraryPage({ data, refresh }: PageProps) {
  ...
  async function bindExisting(event:React.FormEvent<HTMLFormElement>){event.preventDefault();if(!binding)return;const sourceApplicationId=Number(new FormData(event.currentTarget).get("sourceApplicationId")||0);if(!sourceApplicationId)return;setBindingBusy(true);try{await api(`/applications/${binding.applications[0].id}/resume/link`,{method:"POST",body:JSON.stringify({sourceApplicationId}),headers:{"Content-Type":"application/json"}});setBinding(null);await refresh()}catch(reason){window.alert(reason instanceof Error?reason.message:"关联失败")}finally{setBindingBusy(false)}}
  return <>
    ...
    <Panel className="filter-panel"><div className="filter-row"><label className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索文件、公司、岗位或标签" /></label><select className="compact-select" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((value) => <option key={value}>{value}</option>)}</select><span className="result-count">{filtered.filter((group) => group.path).length} 个文件 · {filtered.filter((group) => !group.path).length} 条未绑定</span><div className="view-switch"><button className={view === "list" ? "active" : ""} onClick={() => setView("list")}><List size={15} />列表</button><button className={view === "card" ? "active" : ""} onClick={() => setView("card")}><LayoutGrid size={15} />卡片</button></div></div></Panel>
